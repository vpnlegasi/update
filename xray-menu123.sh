...
BURIQ() {
    curl -sS https://raw.githubusercontent.com/fitri1979/access/main/ip >/root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/fitri1979/access/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION() {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/fitri1979/access/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        Bloman
    else
...
del-user() {
    ...
    exp=$(grep -E "###" "/etc/samvpn/xray/conf/vmess-nontls.json" | grep -w "$user" | awk '{print $3}')
...
backup_vps() {
    ...
    LLatest=$(date)
    Get_Data() {
        git clone https://github.com/fitri1979/user-backup-db.git /root/user-backup/ &>/dev/null
    }
...
" >>/root/user-backup/$Name/$Name-last-backup
    ...
    Save_And_Exit() {
        cd /root/user-backup
        git config --global user.email "fitriomar1979@gmail.com" &>/dev/null
        git config --global user.name "fitri1979" &>/dev/null
        rm -rf .git &>/dev/null
        git init &>/dev/null
        git add . &>/dev/null
        git commit -m m &>/dev/null
        git branch -M main &>/dev/null
        git remote add origin https://github.com/fitri1979/user-backup-db
        git push -f https://ghp_tukar@github.com/fitri1979/user-backup-db.git &>/dev/null
    }
    ...
    link="https://raw.githubusercontent.com/fitri1979/user-backup-db/main/$Name/$Name.zip"
    sleep 1
...
Name=$(curl -sS https://raw.githubusercontent.com/fitri1979/access/main/ip | grep $MYIP | awk '{print $2}')
Exp=$(curl -sS https://raw.githubusercontent.com/fitri1979/access/main/ip | grep $MYIP | awk '{print $3}')
